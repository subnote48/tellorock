...
static wchar_t inputBufferW[1024] = L"";

int main()
{
	boost::asio::io_context io_context;
	boost::asio::ip::udp::socket socket(io_context);
	boost::asio::ip::udp::endpoint listen_endpoint(boost::asio::ip::address::from_string("127.0.0.1"), 8890);
	boost::asio::ip::udp::endpoint connect_endpoint(boost::asio::ip::address::from_string("127.0.0.1"), 8889);
	socket.open(boost::asio::ip::udp::v4());
	SocketTello drone(io_context, socket, listen_endpoint, connect_endpoint);
	drone.StartListening();

	// Simulate the application running
	std::cout << "Listening for UDP responses..." << std::endl;

	UtilTello utilFunction;
	const char* app_version = "0.0.0.1";
	std::string version_string = "Tello Rock@" + std::string(app_version);
	const char* version_cstr = version_string.c_str();

	const char* telloSdkVersion = "1.3.0.0";
	std::string versionSdkString = "Tello SDK" + std::string(telloSdkVersion);
	const char* versionSdk = versionSdkString.c_str();

	spdlog::info(version_cstr);
	spdlog::info(versionSdk);
	utilFunction.AppendToBuffer(version_cstr, inputBuffer);
	utilFunction.AppendToBuffer(versionSdk, inputBuffer);
	...
	g.RegFont = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\SegoeUI.ttf", 33.0f);
	g.MonoFont = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\Consola.ttf", 30.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
			...
			ImGui::ShowDemoWindow(&show_demo_window);
		{
			...
			{
				ImGui::BeginChild("ArrowButtons", { 200, 200 });
				...
				ImGui::BeginChild("RotateButtons", { 210, 70 });
				// Increase the width to accommodate three buttons with spacing, needs less spacing
				ImGui::Columns(3, nullptr, false);
				ImGui::PushButtonRepeat(true);
...

		{
			...
			if (ImGui::Button("Export"))
			{
				...
				bi.lpszTitle = (LPWSTR)"Select a directory";
				// I know this is not good, I can't figure out how to convert a string to LPWSTR without vs studio complaining
...

		{
			...
			ImGui::Text("Tello Rock");
			ImGui::Text("Version: %s", app_version);
			ImGui::Text("Tello SDK: %s", telloSdkVersion);
			ImGui::Text("Funny copy and past.");
			ImGui::Text("By: subnote48");
			if (ImGui::Button("GitHub"))
			{
				ShellExecute(0, 0, L"https://github.com/subnote48/tellorock", 0, 0, SW_SHOW);
			}
			ImGui::End();
		}
